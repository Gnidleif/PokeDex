...
def compileStats(pokemon):
    return {
        ...
        'Immune': ','.join(filter(lambda x: pokemon[x] == '0.0' and x != 'Number', [x for x in pokemon])),
        ...
        'Double': ','.join(filter(lambda x: pokemon[x] == '2.0' and x != 'Number', [x for x in pokemon])),
        'Quadruple': ','.join(filter(lambda x: pokemon[x] == '4.0' and x != 'Number', [x for x in pokemon])),
    }
